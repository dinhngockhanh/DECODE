DECODE_ABCSMCDRF <- function(sample_id = "",
                             mutation_table,
                             criterion = "BIC",
    ...
    threshold_results <- choose_mutation_thresholds_ABCSMCDRF(
    ...
    SFS_convolution_inference_A <- build_convolution_matrix_ABCSMCDRF(
    ...
    SFS_convolution_inference_B <- build_convolution_matrix_ABCSMCDRF(
        sfs_bincount = sfs_bincount,
        mode = "inference B",
        sample_size = matrix_binomial_sample_size,
    ...
    SFS_convolution_validation <- build_convolution_matrix_ABCSMCDRF(
        sfs_bincount = sfs_bincount,
        mode = "validation",
        sample_size = matrix_binomial_sample_size,
    ...
    if (is.na(neutral_tail)) {
        result_with_tail <- DECODE_given_tail_status_ABCSMCDRF(
        ...
        result_without_tail <- DECODE_given_tail_status_ABCSMCDRF(
            SFS_data_inference_A = threshold_results$SFS_data_inference_A,
            SFS_data_inference_B = threshold_results$SFS_data_inference_B,
            SFS_data_validation = threshold_results$SFS_data_validation,
            sfs_bincount = sfs_bincount,
        ...
        final_result <- DECODE_given_tail_status_ABCSMCDRF(
            SFS_data_inference_A = threshold_results$SFS_data_inference_A,
            SFS_data_inference_B = threshold_results$SFS_data_inference_B,
            SFS_data_validation = threshold_results$SFS_data_validation,
            sfs_bincount = sfs_bincount,
    ...
    # tmp <- parameter_conversion_ABCSMCDRF(
    #     result = final_result,
...
DECODE_given_tail_status_ABCSMCDRF <- function(SFS_data_inference_A,
    ...
    while (TRUE) {
        ...
        fit_results <- DECODE_given_tail_status_and_Ncluster_ABCSMCDRF(
...
DECODE_given_tail_status_and_Ncluster_ABCSMCDRF <- function(SFS_data_inference_A,
                                                            SFS_data_inference_B,
                                                            SFS_data_validation,
                                                            N_humps,
                                                            with_tail,
    ...
    #---Function to compute SFS given parameter sets
    ...
    func_ABCSMCDRF_trial <- function(with_tail,
                                     neutral_power,
                                     cluster_frequencies,
                                     cluster_proportions_inference_A,
                                     sfs_bincount,
                                     N_end,
                                     SFS_convolution_matrix_inference_A,
                                     SFS_convolution_matrix_inference_B) {
        #   Build the SFS component libraries
        component_distributions_inference_A <- build_SFS_library_ABCSMCDRF(
        ...
        component_distributions_inference_B <- build_SFS_library_ABCSMCDRF(
            neutral_power = neutral_power,
            cluster_frequencies = cluster_frequencies,
            sfs_bincount = sfs_bincount,
        ...
        #   Compute predicted SFS for each inference set
        predicted_SFS_inference_A <- compute_SFS_ABCSMCDRF(
        ...
        predicted_SFS_inference_A <- predicted_SFS_inference_A / sum(predicted_SFS_inference_A)
        predicted_SFS_inference_B <- compute_SFS_ABCSMCDRF(
        ...
        predicted_SFS_inference_B <- predicted_SFS_inference_B / sum(predicted_SFS_inference_B)
        #   Return results from this trial
        output <- list()
        output$predicted_SFS_inference_A <- predicted_SFS_inference_A
        output$predicted_SFS_inference_B <- predicted_SFS_inference_B
        return(output)
    }
    #---Function to produce training set for ABC-SMC-DRF
    model <- function(parameters, parallel = compute_parallel) {
        #   Compute corresponding SFS for each parameter set
        if (parallel) {
            library(parallel)
            library(pbapply)
            library(data.table)
            ...
            clusterExport(cl, varlist = c(
                "with_tail", "sfs_bincount", "N_end",
                "SFS_convolution_matrix_inference_A", "SFS_convolution_matrix_inference_B",
                "build_SFS_library_ABCSMCDRF", "build_SFS_library_Griffiths_Tavare_ABCSMCDRF", "compute_SFS_ABCSMCDRF"
            ), envir = environment())
            stats <- parLapply(
                cl = cl, X = 1:nrow(parameters),
                fun = function(i) {
                    predicted_SFS <- func_ABCSMCDRF_trial(
                        with_tail = with_tail,
                        neutral_power = ifelse(with_tail, as.numeric(parameters[i, "neutral_power"]), NA),
                        cluster_frequencies = as.numeric(parameters[i, paste0("cluster_frequency_", 1:N_humps)]),
                        cluster_proportions_inference_A = as.numeric(parameters[i, paste0("cluster_proportion_inference_A_", 1:(N_humps + with_tail))]),
                        sfs_bincount = sfs_bincount,
                        N_end = N_end,
                    ...
                    stats <- data.frame(matrix(c(predicted_SFS$predicted_SFS_inference_A, predicted_SFS$predicted_SFS_inference_B), nrow = 1))
                    return(stats)
                }
            )
            stopCluster(cl)
            ...
            for (i in 1:nrow(parameters)) {
                ...
                stats <- rbind(stats, c(predicted_SFS$predicted_SFS_inference_A, predicted_SFS$predicted_SFS_inference_B))
            }
            stats <- data.frame(stats)
        }
        colnames(stats) <- c(paste0("SFS_inference_A_", 1:sfs_bincount), paste0("SFS_inference_B_", 1:sfs_bincount))
        stats <- cbind(parameters, stats)
        return(stats)
    }
    #---Inference target = normalized SFS from data
    statistics_target <- data.frame(matrix(c(SFS_data_inference_A / sum(SFS_data_inference_A), SFS_data_inference_B / sum(SFS_data_inference_B)), nrow = 1))
    colnames(statistics_target) <- c(paste0("SFS_inference_A_", 1:sfs_bincount), paste0("SFS_inference_B_", 1:sfs_bincount))
    #---Prior distributions and bounds for each parameter
    parameters_initial <- data.frame(matrix(ncol = 0, nrow = N_trials))



    parameters_initial[paste0("cluster_proportion_inference_A_", 1:(N_humps + with_tail))] <- rdirichlet(n = N_trials, alpha = rep(1, N_humps + with_tail))
    # parameters_initial[paste0("cluster_proportion_inference_A_", 1:(N_humps + with_tail))] <- runif(N_trials * (N_humps + with_tail), 0, 1)
    # parameters_initial[, paste0("cluster_proportion_inference_A_", 1:(N_humps + with_tail))] <- t(apply(parameters_initial[, paste0("cluster_proportion_inference_A_", 1:(N_humps + with_tail))], 1, function(x) x / sum(x)))

    bounds <- data.frame(
        parameter = paste0("cluster_proportion_inference_A_", 1:(N_humps + with_tail)),
        min = 0, max = 1
    )
    # bounds <- data.frame(
    #     parameter = paste0("cluster_proportion_inference_A_", 1:(N_humps + with_tail)),
    #     min = 0.01, max = 1
    # )
    parameters_initial[paste0("cluster_frequency_", 1:N_humps)] <- matrix(runif(N_trials * N_humps, cluster_frequency_min, cluster_frequency_max), nrow = N_trials)
    parameters_initial[paste0("cluster_frequency_", 1:N_humps)] <- t(apply(parameters_initial[paste0("cluster_frequency_", 1:N_humps)], 1, sort, decreasing = TRUE))
    bounds <- rbind(bounds, data.frame(
        parameter = paste0("cluster_frequency_", 1:N_humps),
        min = cluster_frequency_min, max = cluster_frequency_max
    ))
    if (with_tail) {
        parameters_initial["neutral_power"] <- runif(N_trials, neutral_power_min, neutral_power_max)
        bounds <- rbind(bounds, data.frame(
            parameter = "neutral_power",
            min = neutral_power_min, max = neutral_power_max
        ))
    }
    #---Run ABC-SMC-DRF to find posterior distributions
    # abcrf_results <- smcrf_single_param(
    #     statistics_target = statistics_target,
    #     model = model,
    #     bounds = bounds,
    #     parameters_initial = parameters_initial,
    #     nParticles = rep(5000, 10),
    #     parallel = TRUE
    # )
    abcrf_results <- smcrf_multi_param(
        statistics_target = statistics_target,
        model = model,
        bounds = bounds,
        parameters_initial = parameters_initial,
    ...
    ####################################################################
    ####################################################################
    reference <- abcrf_results[[paste0("Iteration_", abcrf_results$nIterations)]]$reference
    weights <- abcrf_results[[paste0("Iteration_", abcrf_results$nIterations)]]$weights
    return()
    ####################################################################
    ####################################################################
    ####################################################################
    ...
    if (compute_parallel == FALSE) {
        ...
        for (i in 1:N_trials) {
            ...
            trial_result <- func_ABCSMCDRF_trial(
                with_tail = with_tail,
                N_humps = N_humps,
                neutral_power_min = neutral_power_min,
                neutral_power_max = neutral_power_max,
        ...
        clusterExport(cl, varlist = c(
            ...
            "build_SFS_library_ABCSMCDRF", "build_SFS_library_Griffiths_Tavare_ABCSMCDRF",
            "compute_loglikelihood_ABCSMCDRF", "compute_SFS_ABCSMCDRF"
        ), envir = environment())
    ...
    for (posterior_trial in 1:N_trials_kept) {
        ...
        component_distributions_validation <- build_SFS_library_ABCSMCDRF(
            neutral_power = neutral_power,
            cluster_frequencies = cluster_frequencies,
            sfs_bincount = sfs_bincount,
    ...
    criteria_results <- cluster_count_criteria_ABCSMCDRF(
...
choose_mutation_thresholds_ABCSMCDRF <- function(mutation_table,
                                                 max_total_read,
...
cluster_count_criteria_ABCSMCDRF <- function(num_parameters,
                                             SFS_data_validation,
                                             parameters,
    ...
    for (i_parameter in 1:nrow(parameters)) {
        ...
        for (i_validation in 1:nrow(SFS_data_validation)) {
            logLikelihood <- compute_loglikelihood_ABCSMCDRF(
                A = A,
                vec_K = vec_K,
...
parameter_conversion_ABCSMCDRF <- function(result,
...
compute_loglikelihood_ABCSMCDRF <- function(A, vec_K, component_distributions, vec_SFS_real, zero_cutoff) {
    #----------------Compute the SFS probability distribution from model
    vec_SFS_model <- compute_SFS_ABCSMCDRF(
        A = A,
        vec_K = vec_K,
...
compute_SFS_ABCSMCDRF <- function(A, vec_K, component_distributions) {
...
build_SFS_library_ABCSMCDRF <- function(neutral_power, cluster_frequencies, sfs_bincount, SFS_convolution_matrix, N_end) {
    SFS_exact <- c()
    SFS_expected <- c()
    ...
    if (is.na(neutral_power)) {
        ...
        vec_SFS_GT <- build_SFS_library_Griffiths_Tavare_ABCSMCDRF(vec_para = vec_para, N_end = N_end)
        vec_SFS_expected <- rep(0, sfs_bincount)
...
build_SFS_library_Griffiths_Tavare_ABCSMCDRF <- function(vec_para, N_end) {
    #---Get the parameters
...
build_convolution_matrix_ABCSMCDRF <- function(sfs_bincount,
                                               mode = NULL,
                                               sample_size,
                                               min_variant_read,
                                               min_total_read,
