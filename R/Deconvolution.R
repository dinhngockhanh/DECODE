SFS_deconvolution <- function(mutation_table,
    ...
    if (compute_parallel == FALSE) {
        ...
        for (i in seq_along(list_frequencies_tmp)) {
            ...
            if (p <= 0) {
                vec_para <- c(1, -p)
                output <- build_SFS_library(vec_para)
        ...
        build_SFS_library <<- build_SFS_library
        build_SFS_library_Griffiths_Tavare <<- build_SFS_library_Griffiths_Tavare
        ...
        output <- pblapply(cl = cl, X = 1:length(list_frequencies_tmp), FUN = function(i) {
            ...
            return(build_SFS_library(vec_para))
        })
        for (i in seq_along(list_frequencies_tmp)) {
            p <- list_frequencies_tmp[i]
            if (p <= 0) {
...
build_SFS_library <- function(vec_para) {
