DECODE_plot_model_selection_ABCSMCDRF <- function(DECODE_result,
                                                  fit = "best",
                                                  mode = "inference_A",
                                                  SFS_limit = TRUE,
    ...
    func_one_fit <- function(p_right, fit, text, text_color) {
        ...
        p_right_inference_A <- DECODE_plot_SFS_ABCSMCDRF(
        ...
        p_right_inference_B <- DECODE_plot_SFS_ABCSMCDRF(
            DECODE_result = DECODE_result,
            fit = fit,
            mode = "inference_B",
            DECODE_linewidth = 1,
        ...
        p_right_validation <- DECODE_plot_SFS_ABCSMCDRF(
            DECODE_result = DECODE_result,
            fit = fit,
            mode = "validation",
            DECODE_linewidth = 1,
    ...
    if ("fits_with_tail" %in% names(DECODE_result)) {
        ...
        for (fit in names(DECODE_result$fits_with_tail$all_fits)) {
            ...
            criteria_new <- DECODE_result$fits_with_tail$all_fits[[fit]]$summary$criteria
    ...
    if ("fits_without_tail" %in% names(DECODE_result)) {
        ...
        for (fit in names(DECODE_result$fits_without_tail$all_fits)) {
            ...
            criteria_new <- DECODE_result$fits_without_tail$all_fits[[fit]]$summary$criteria
            criteria_new$fit <- fit_ID
    ...
    p_left_threshold_selection <- DECODE_plot_readcounts_ABCSMCDRF(
        DECODE_result = DECODE_result
    )
    if (("fits_with_tail" %in% names(DECODE_result)) & ("fits_without_tail" %in% names(DECODE_result))) {
        p_left_criteria_with_tail <- DECODE_plot_criteria_ABCSMCDRF(
        ...
        p_left_criteria_without_tail <- DECODE_plot_criteria_ABCSMCDRF(
...
DECODE_plot_criteria_ABCSMCDRF <- function(criteria,
                                           criterion,
                                           criterion_ratio,
                                           fit_colors,
                                           legend = TRUE) {
    ...
    y_min <- min(c(min(criteria[[criterion]] - criteria[[paste0(criterion, "_sd")]], na.rm = TRUE), min(criteria[[paste0(criterion, "_target")]], na.rm = TRUE)))
    y_max <- max(c(max(criteria[[criterion]] + criteria[[paste0(criterion, "_sd")]], na.rm = TRUE), max(criteria[[paste0(criterion, "_target")]], na.rm = TRUE)))
    p <- ggplot(criteria) +
        geom_point(aes(x = fit, y = !!sym(criterion), shape = "Actual", color = note), size = 10) +
        geom_linerange(aes(x = fit, ymin = !!sym(criterion) - !!sym(paste0(criterion, "_sd")), ymax = !!sym(criterion) + !!sym(paste0(criterion, "_sd")), color = note), size = 2) +
...
DECODE_plot_SFS_ABCSMCDRF <- function(DECODE_result,
                                      fit = "best",
                                      mode = "inference_A",
                                      DECODE_linewidth = 5,
    ...
    if (mode == "inference_A") {
        ...
        if (fit == "best") {
            parameters <- DECODE_result$final_fit$best_fit$parameters
            ...
            parameters <- DECODE_result[[detail_tail]]$all_fits[[detail_Ncluster]]$summary$parameters
            tail_status <- ifelse(detail_tail == "fits_with_tail", TRUE, FALSE)
            component_distributions <- DECODE_result[[detail_tail]]$all_fits[[detail_Ncluster]]$summary$component_distributions_inference_A
        }
        plot_parameters <- data.frame(
            Tail = parameters$Tail,
            Tail_proportion = parameters$Tail_proportion_inference_A,
            Tail_power = parameters$Tail_power,
            Cluster_count = parameters$Cluster_count
        )
        for (i in 1:plot_parameters$Cluster_count) {
            plot_parameters[[paste0("Cluster_proportion_", i)]] <- parameters[[paste0("Cluster_proportion_inference_A_", i)]]
            plot_parameters[[paste0("Cluster_VAF_", i)]] <- parameters[[paste0("Cluster_VAF_", i)]]
        }
        ...
        if (fit == "best") {
            ...
            parameters <- DECODE_result[[detail_tail]]$all_fits[[detail_Ncluster]]$summary$parameters
            tail_status <- ifelse(detail_tail == "fits_with_tail", TRUE, FALSE)
            component_distributions <- DECODE_result[[detail_tail]]$all_fits[[detail_Ncluster]]$summary$component_distributions_inference_B
        }
        plot_parameters <- data.frame(
            Tail = parameters$Tail,
            Tail_proportion = parameters$Tail_proportion_inference_B,
            Tail_power = parameters$Tail_power,
            Cluster_count = parameters$Cluster_count
        )
        for (i in 1:plot_parameters$Cluster_count) {
            plot_parameters[[paste0("Cluster_proportion_", i)]] <- parameters[[paste0("Cluster_proportion_inference_B_", i)]]
            plot_parameters[[paste0("Cluster_VAF_", i)]] <- parameters[[paste0("Cluster_VAF_", i)]]
        }
        ...
        if (fit == "best") {
            ...
            parameters <- DECODE_result[[detail_tail]]$all_fits[[detail_Ncluster]]$summary$parameters
            tail_status <- ifelse(detail_tail == "fits_with_tail", TRUE, FALSE)
            component_distributions <- DECODE_result[[detail_tail]]$all_fits[[detail_Ncluster]]$summary$component_distributions_validation
            criteria_validation_index <- DECODE_result[[detail_tail]]$all_fits[[detail_Ncluster]]$summary$criteria_validation_index
        }
        plot_parameters <- data.frame(
            Tail = parameters$Tail,
            Tail_proportion = parameters$Tail_proportion_validation,
            Tail_power = parameters$Tail_power,
            Cluster_count = parameters$Cluster_count
        )
        for (i in 1:plot_parameters$Cluster_count) {
            plot_parameters[[paste0("Cluster_proportion_", i)]] <- parameters[[paste0("Cluster_proportion_validation_", i)]]
            plot_parameters[[paste0("Cluster_VAF_", i)]] <- parameters[[paste0("Cluster_VAF_", i)]]
        }
        vec_SFS_real <- colMeans(vec_SFS_real)
    }
    if (tail_status) {
        vec_A <- c(plot_parameters$Tail_proportion, plot_parameters$Tail_power)
    } else {
        vec_A <- c(NA, NA)
    }
    N_humps <- plot_parameters$Cluster_count
    if (N_humps == 0) {
        vec_p <- c()
        vec_K <- c()
    } else {
        ...
        vec_K <- c()
        for (i in 1:N_humps) {
            vec_p <- c(vec_p, plot_parameters[[paste0("Cluster_VAF_", i)]])
            vec_K <- c(vec_K, plot_parameters[[paste0("Cluster_proportion_", i)]])
        }
    ...
    df_fit_order <- c()
    if (tail_status) {
        SFS_neutral <- compute_SFS_ABCSMCDRF(
            A = vec_A[1],
            vec_K = c(),
...
DECODE_plot_readcounts_ABCSMCDRF <- function(DECODE_result,
                                             freq_cutoff = 10) {
    suppressPackageStartupMessages(library(ggplot2))
...
analysis_CINner_ABCSMCDRF <- function(groundtruth_df,
                                      mobster_df = NULL,
                                      decode_df = NULL,
                                      text_notation = FALSE,
                                      cluster_count = NA,
...
plot_analysis_ABCSMCDRF <- function(results,
                                    algorithm_colors,
                                    cohort,
                                    cluster_shapes,
                                    cluster_labels,
                                    ...
                                    cohort_colors,
                                    folder_workplace) {
    # .........modify from analysis_ICGC...........
}

analysis_ICGC_ABCSMCDRF <- function(sample_information_df,
                                    mobster_df = NULL,
                                    decode_df = NULL,
...
analysis_DREAM_ABCSMCDRF <- function(sample_information_df,
                                     sample_excel_df,
                                     decode_df = NULL,
                                     decode_BIC_df = NULL,
                                     text_notation = FALSE,
