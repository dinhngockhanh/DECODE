DECODE_plot_model_selection_ABC <- function(DECODE_result,
                                            fit = "best",
                                            mode = "inference_A",
                                            SFS_limit = TRUE,
    ...
    func_one_fit <- function(p_right, fit, text, text_color) {
        ...
        p_right_inference_A <- DECODE_plot_SFS_ABC(
        ...
        p_right_inference_B <- DECODE_plot_SFS_ABC(
            DECODE_result = DECODE_result,
            fit = fit,
            mode = "inference_B",
            DECODE_linewidth = 1,
        ...
        p_right_validation <- DECODE_plot_SFS_ABC(
            DECODE_result = DECODE_result,
            fit = fit,
            mode = "validation",
            DECODE_linewidth = 1,
    ...
    p_left_threshold_selection <- DECODE_plot_readcounts_ABC(
        DECODE_result = DECODE_result
    )
    if (("fits_with_tail" %in% names(DECODE_result)) & ("fits_without_tail" %in% names(DECODE_result))) {
        p_left_criteria_with_tail <- DECODE_plot_criteria_ABC(
        ...
        p_left_criteria_without_tail <- DECODE_plot_criteria_ABC(
...
DECODE_plot_criteria_ABC <- function(criteria,
                                     criterion,
                                     criterion_ratio,
                                     fit_colors,
                                     legend = TRUE) {
...
DECODE_plot_SFS_ABC <- function(DECODE_result,
                                fit = "best",
                                mode = "inference_A",
                                DECODE_linewidth = 5,
...
DECODE_plot_readcounts_ABC <- function(DECODE_result,
                                       freq_cutoff = 10) {
    suppressPackageStartupMessages(library(ggplot2))
...
analysis_CINner_ABC <- function(groundtruth_df,
                                mobster_df = NULL,
                                decode_df = NULL,
                                text_notation = FALSE,
                                cluster_count = NA,
...
plot_analysis_ABC <- function(results,
                              algorithm_colors,
                              cohort,
                              cluster_shapes,
                              cluster_labels,
...
analysis_ICGC_ABC <- function(sample_information_df,
                              mobster_df = NULL,
                              decode_df = NULL,
...
analysis_DREAM_ABC <- function(sample_information_df,
                               sample_excel_df,
                               decode_df = NULL,
                               decode_BIC_df = NULL,
                               text_notation = FALSE,
