simulation_clonal_evolution <- function(t_end_time = 0,
                                        ...
                                        vec_cell_lifespan = Inf,
                                        vec_division_probability = 0,
                                        range_clonal_perc = 0,
    ...
    while (t_now < t_end_time & sum(vec_populations) > 0) {
        ...
        vec_propensity <- t_tau_step * vec_populations / vec_cell_lifespan
        ...
        while (max(vec_count_division + vec_count_death - vec_populations) > 0) {
            vec_count_event <- rpois(
                n = (n_selective_clones + 1),
                lambda = vec_propensity
            )
            vec_count_division <- rbinom(
                n = length(vec_count_event),
                size = vec_count_event,
                prob = vec_division_probability
            )
            vec_count_death <- vec_count_event - vec_count_division
        }
...
simulation_clonal_evolution_old <- function(t_end_time = 0,
                                            n_selective_clones = 1,
                                            vec_s_mut_time = 0,
                                            vec_s_mut_hierarchy = 0,
                                            vec_propensity = 0,
