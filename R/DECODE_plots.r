...
DECODE_plot_readcounts <- function(mutation_table,
                                   freq_cutoff = 0.1,
        ...
        for (min_total_read in vec_min_total_read) {
            df <- rbind(
                ...
                data.frame(
                    ...
                    freq = sum(mutation_table$Alt_count >= min_variant_read & mutation_table$Tot_count >= min_total_read) / nrow(mutation_table)
                )
            )
        }
        return(df)
